...
describe("Robots.txt Validation", () => {
  ...
  it("should have valid robots.txt content", () => {
    if (!fs.existsSync(robotsPath)) {
      throw new Error(
        "robots.txt file not found in dist directory. Run build first.",
      );
    }
    ...
    expect(content).toMatch(
      /Sitemap:\s*https:\/\/gui\.do\/sitemap-index\.xml/i,
    );
  });
  ...
  it("should ensure website is indexable (not blocked site-wide)", () => {
    ...
    for (const line of lines) {
      ...
      if (
        currentUserAgent.match(/User-agent:\s*\*/i) &&
        line.match(/^Disallow:\s*\/$/i)
      ) {
