...
function collectUrls(): UrlToValidate[] {
  ...
  const recentEvents = (events as Array<{ url?: string; name: string }>).slice(
    0,
    20,
  );
  ...
  try {
    ...
    if (response.status === 405) {
      ...
      if (getResponse.status >= 400) {
        console.warn(
          `  Warning: ${url} returned ${getResponse.status} (may be bot blocking)`,
        );
        return null;
      }
      return null;
    ...
    if (response.status >= 400) {
      console.warn(
        `  Warning: ${url} returned ${response.status} (may be bot blocking)`,
      );
      return null;
    }
...
describe("Data File URL Validation", () => {
  it("should have valid URLs in indieProjects", async () => {
    const urls = collectUrls().filter((u) =>
      u.source.startsWith("indieProjects"),
    );
  ...
  it("should have valid URLs in socialLinks", async () => {
    const urls = collectUrls().filter((u) =>
      u.source.startsWith("socialLinks"),
    );
  ...
  it("should have valid URLs in recent pressCoverage", async () => {
    const urls = collectUrls().filter((u) =>
      u.source.startsWith("pressCoverage"),
    );
