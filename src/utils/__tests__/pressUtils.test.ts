...
describe('pressUtils', () => {
  ...
  describe('getFaviconUrl', () => {
    it('returns Google favicon service URL for valid URLs', () => {
      ...
      expect(result).toBe('https://www.google.com/s2/favicons?domain=example.com&sz=32&default=/assets/document-icon.svg');
    });
    ...
    it('extracts domain correctly from complex URLs', () => {
      ...
      urls.forEach(url => {
        ...
        const result = getFaviconUrl(url);
        expect(result).toBe(`https://www.google.com/s2/favicons?domain=${domain}&sz=32&default=/assets/document-icon.svg`);
      });
    });
