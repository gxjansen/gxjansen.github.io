...
export function isPresentation(value: unknown): value is Presentation {
  ...
  if (
    !presentation.relatedEventSlugs.every((slug) => typeof slug === "string")
  ) {
    return false;
  }
...
export function validatePresentation(presentation: unknown): string[] {
  ...
  if (!p.duration?.trim())
    errors.push("duration field is required and must not be empty");
  if (typeof p.isWorkshop !== "boolean")
    errors.push("isWorkshop field must be a boolean");
  if (typeof p.isFeatured !== "boolean")
    errors.push("isFeatured field must be a boolean");
  if (!p.image?.trim())
    errors.push("image field is required and must not be empty");
  ...
  if (!Array.isArray(p.relatedEventSlugs)) {
    ...
    if (
      !p.relatedEventSlugs.every(
        (slug) => typeof slug === "string" && slug.trim(),
      )
    ) {
      errors.push(
        "relatedEventSlugs field must contain only non-empty strings",
      );
    }
  }
