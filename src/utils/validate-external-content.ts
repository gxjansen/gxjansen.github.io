...
async function validateSpotifyEpisode(
  episodeId: string,
): Promise<ValidationResult> {
  const result: ValidationResult = {
...
export async function validateExternalContent(
  ...
  for (const item of items) {
    ...
    if (item.youtubeLink) {
      ...
      console.log(
        `✓ Checked YouTube: ${item.title} - ${result.available ? "Available" : "Unavailable"}`,
      );
    }
    ...
    if (item.spotifyEmbedId) {
      ...
      console.log(
        `✓ Checked Spotify: ${item.title} - ${result.available ? "Available" : "Unavailable"}`,
      );
    }
    ...
    if (item.articleUrl && !item.youtubeLink && !item.spotifyEmbedId) {
      ...
      console.log(
        `✓ Checked Article: ${item.title} - ${result.available ? "Available" : "Unavailable"}`,
      );
    }
