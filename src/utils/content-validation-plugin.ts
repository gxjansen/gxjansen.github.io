...
} from "./validate-external-content";
      ...
      "astro:build:start": async ({ logger }) => {
        ...
        try {
          ...
          const shouldValidate =
            process.env.VALIDATE_CONTENT === "true" ||
          ...
          if (report.unavailable > 0) {
            logger.warn(
              `Found ${report.unavailable} unavailable content items`,
            );
            ...
            logger.info(
              `Detailed unavailable content log saved to: ${logPath}`,
            );
          } else {
