...
**Lexicons** define the data structure. I've registered the `forum.barazo.*` namespace with record types for topics, replies, reactions, and user preferences. Think of them as the shared contract between all the pieces. Published as an [open source TypeScript package](https://github.com/barazo-forum/barazo-lexicons).

**The AppView** (the [backend API](https://github.com/barazo-forum/barazo-api)) subscribes to the AT Protocol firehose through [Tap](https://github.com/bluesky-social/indigo), filtering for `forum.barazo.*` records. It indexes everything into PostgreSQL for fast reads, handles authentication via AT Protocol OAuth, and exposes a REST API for all forum operations.

**The frontend** ([Next.js app](https://github.com/barazo-forum/barazo-web)) consumes that API and renders the forum. Server-side rendered, accessible (targeting WCAG 2.2 AA), with full-text search, moderation tools, notifications, and an admin panel.
...
- [barazo-lexicons](https://github.com/barazo-forum/barazo-lexicons): AT Protocol schemas + generated TypeScript types (MIT)
- [barazo-api](https://github.com/barazo-forum/barazo-api): Fastify backend with OAuth, firehose, full REST API (AGPL-3.0)
- [barazo-web](https://github.com/barazo-forum/barazo-web): Next.js frontend (MIT)
- [barazo-deploy](https://github.com/barazo-forum/barazo-deploy): Docker Compose for self-hosting (MIT)
...
If you write code: the [repos are open](https://github.com/orgs/barazo-forum/repositories). Issues, PRs, architectural feedback... all welcome.
