...
So yeah, let's not do that. But you still have some very powerful options for customizing pre-trained models are either free or at least far below $100M... 👇

## 1. Stuff your prompts with context

The most accessible way to customize LLM behavior is through prompt engineering. This involves crafting specific instructions that guide the model's responses. There are thousands of ways to structure prompts (and YouTube video's on how to do it), but specifically for enriching your LLM with context you can add/upload documents, files, or even just plain text.
...
**Tip 1**: You can (also) provide examples of what the end result should look like. If you want the a to use a certain tone of voice, structure text in a certain way or always include certain elements: instead of only **telling** it to do so, it really helps if you can actually show it what a desired outcome looks like. This method does NOT change the models' knowledge base or weights or anything, but it can definitely help the model understand the desired behavior and tune it towards your needs.
...
Fine-tining does adjust the model's weights to better align with your use case and/or based on any specific data that you provide. Requires substantial data, can still be expensive, will need re-training with every update. 

## 3. RAG (Retrieval-Augmented Generation)

Retrieves relevant information from external knowledge base and dynamically incorporates it. Allows you to go beyond context windows, doesn't require re-training when the data changes. So if your data source is up-to-date, so will your LLM. Does require you to have a vector database and retrieval system.
...
This is a connector/interface/tool the LLM can use. Example: connect to Google Docs or Asana to search for documents or add tasks. IF the API allows the action, your LLM can now use or even do it.
...
On Github you find [https://github.com/modelcontextprotocol](a list of MCPs that are available), but you can also "simply" ask your AI to create it's own MCP tool if you use it in a tool that allows this (like Cline for VSCode)
...
For simplicity and flexibility, I usually use a combination of #1️ (adjusting my prompts) and #4️ (MCPs).
