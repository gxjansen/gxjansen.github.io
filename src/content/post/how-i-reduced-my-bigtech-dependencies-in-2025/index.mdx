...
- Webflow → [Astro](https://astro.build/) (on Netlify)
...
After that:
- As long as the Synologies still function, I don't see why I should replace them, at least not their NAS function. I do have more and more Homelab containers running which is beginning to stretch its limits so a dedicated docker server might be in my future.
- Replace Tailscale with Headscale and Bitwarden with Vaultwarden? Not sure if that these a worth the hassle.
- Setup FreshRSS? Still haven't recovered from the Google Reader sunset 😅
